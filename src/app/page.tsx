...
import { copy, foodName, foodSubtitle, priceLabel, type Language, type MealType, type FoodCategory } from '@/lib/i18n';
...
import { detectCurrentMeal, isMealType } from '@/lib/meal-time';
...
import { ArrowUpRight, AudioLines, Volume2, VolumeX, Sparkles, Utensils, CupSoda, Leaf } from 'lucide-react';
...
function FoodImage({ food, language }: { food: Food; language: Language }) {
  if (food.customId || food.image < 0) {
    const isDrink = food.category === 'drink';
    return <div className="food-image custom-food-art" role="img" aria-label={foodName(food, language)}>{isDrink ? <CupSoda size={64} /> : <Utensils size={64} />}</div>;
  }
  ...
  if (food.image >= 300) {
    const offset = food.image - 300;
    atlas = `drinks-${Math.floor(offset / 12)}`;
    index = offset % 12;
  } else if (food.image >= 200) {
    const offset = food.image - 200;
    atlas = `food-new-${Math.floor(offset / 12)}`;
    index = offset % 12;
  } else if (food.image >= 120) {
    isCommon = true;
    atlas = `food-common-${Math.floor((food.image - 120) / 12)}`;
    index = (food.image - 120) % 12;
  } else if (food.image >= 72) {
    isLunch = true;
    atlas = `food-lunch-${Math.floor((food.image - 72) / 12)}`;
    index = (food.image - 72) % 12;
  } else if (food.image >= 36) {
    atlas = `food-expanded-${Math.floor((food.image - 36) / 12)}`;
    index = (food.image - 36) % 12;
  } else {
    isExpanded = false;
    atlas = `food-hd-${Math.floor(food.image / 4)}`;
    index = food.image % 4;
  }

  if (!AVAILABLE_ATLASES.has(atlas)) {
    const isDrink = food.category === 'drink' || food.image >= 300;
    return <div className="food-image custom-food-art" role="img" aria-label={foodName(food, language)}>{isDrink ? <CupSoda size={64} /> : <Utensils size={64} />}</div>;
  }

  const yPositions = isCommon ? [0, 50, 100] : isLunch ? [0, 46, 92] : [0, 50, 100];
  const clip = isCommon ? "inset(0 0 4% 0)" : isLunch ? "inset(0 0 7% 0)" : undefined;

  return <div role="img" aria-label={foodName(food, language)} className="food-image" style={{ clipPath: clip, backgroundImage: `url(${basePath}/${atlas}.webp)`, backgroundSize: isExpanded ? '400% 300%' : '200% 200%', backgroundPosition: isExpanded ? `${(index % 4) / 3 * 100}% ${yPositions[Math.floor(index / 4)]}%` : `${(index % 2) * 100}% ${Math.floor(index / 2) * 100}%` }} />
}
...
export default function Home() {
  ...
  const [meal, setMeal] = useState<MealType>(() => detectCurrentMeal());
  ...
  useEffect(() => {
    ...
    let initialCategory: FoodCategory = 'food';
    try {
      const savedCat = readCookie<string>('category');
      if (savedCat === 'food' || savedCat === 'drink') {
        initialCategory = savedCat;
        setCategory(savedCat);
      }
    } catch { }

    let initialMeal: MealType = detectCurrentMeal();
    try {
      const savedMeal = readCookie<string>('meal');
      if (isMealType(savedMeal)) {
        initialMeal = savedMeal;
        setMeal(savedMeal);
      }
    } catch { }

    const tCopy = copy[selected];
    document.title = tCopy.pageTitles[initialCategory]?.[initialMeal] || tCopy.pageTitles.food[initialMeal] || tCopy.pageTitles.food.lunch;
  }, []);
  const changeLanguage = (next: Language) => {
    setLanguage(next);
    document.documentElement.lang = next;
    const tCopy = copy[next];
    document.title = tCopy.pageTitles[category]?.[meal] || tCopy.pageTitles.food[meal] || tCopy.pageTitles.food.lunch;
    try { writeCookie('language', next) } catch { }
  };
  const handleCategoryChange = (nextCat: FoodCategory) => {
    setCategory(nextCat);
    const tCopy = copy[language];
    document.title = tCopy.pageTitles[nextCat]?.[meal] || tCopy.pageTitles.food[meal] || tCopy.pageTitles.food.lunch;
    try { writeCookie('category', nextCat) } catch { }
  };
  const handleMealChange = (nextMeal: MealType) => {
    setMeal(nextMeal);
    const tCopy = copy[language];
    document.title = tCopy.pageTitles[category]?.[nextMeal] || tCopy.pageTitles.food[nextMeal] || tCopy.pageTitles.food.lunch;
    try { writeCookie('meal', nextMeal) } catch { }
  };
  ...
  const eligible = useMemo(() => population.filter(f => (!veg || f.veg) && (f.category || 'food') === category && (f.category === 'drink' || !f.meals || f.meals.length === 0 || f.meals.includes(meal))), [population, veg, category, meal]);
